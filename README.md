# Minecraft
Atividade Aula-05 Desenvolvimento Front-End para web.

# Projeto Minecraft - HTML Puro

Projeto acadêmico desenvolvido somente com HTML, sem CSS e sem JavaScript.

## Estrutura
- `html/`: 12 páginas HTML interligadas por um menu presente em todas.
- `img/`: imagens usadas com `<img>`, `<figure>` e `<figcaption>`.
- `audio/`: áudio local usado com `<audio controls>`.
- `video/`: vídeo local usado com `<video controls>`.

## Páginas
1. index.html
2. historia.html
3. modos-de-jogo.html
4. sobrevivencia.html
5. criativo.html
6. mobs.html
7. blocos.html
8. biomas.html
9. dimensoes.html
10. ferramentas.html
11. curiosidades.html
12. cadastro.html

## Conteúdos HTML utilizados
Tags semânticas (`header`, `nav`, `main`, `section`, `article`, `aside`, `footer`), listas, links, tabelas, formulários, `fieldset`, `legend`, `label`, áudio, vídeo, `figure`, `figcaption`, inputs `date`, `file`, `color` e `range`, `required`, `placeholder`, `datalist`, `details`, `summary`, `mark`, `del`, `ins`, `abbr`, `blockquote`, `progress` e `meter`.

## Mídias
Antes da entrega, coloque arquivos reais nas pastas mantendo estes nomes:
- `img/minecraft-capa.jpg`
- `img/blocos.jpg`
- `img/video-capa.jpg`
- `audio/minecraft-audio.mp3`
- `video/minecraft-video.mp4`

Abra `html/index.html` para iniciar o projeto.
